...
async function executeTool(name, args) {
  ...
  if (name === "run_powershell" && allowShell) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timeout = setTimeout(() => child.kill(), 24 * 60 * 60 * 1000);
...
async function runTurn(userPrompt) {
  ...
  for (let turn = 0; runUntilComplete || turn < maxRounds; turn += 1) {
    ...
    const data = await request({
      ...
      max_tokens: answerOnly ? 1024 : 8192,
      temperature: 0.2,
    ...
    writeSessionEvent("assistant_response", {
      ...
      generation: { thinking_requested: autonomous || (enableThinking && !answerOnly), thinking_enabled: autonomous || (enableThinking && !answerOnly), max_tokens: answerOnly ? 1024 : 8192, finish_reason: data.choices?.[0]?.finish_reason, usage: data.usage },
    });
    ...
    if (!calls.length) {
      ...
      if (autonomous && job.status !== "completed_candidate") {
        ...
        if (truncatedAction) {
          ...
          writeSessionEvent("loop_recovery", { turn_id: turnId, round: turn + 1, reason: "truncated_structured_action" });
            ...
            if (threeCaTools.handles(name)) {
              try {
                ...
                for (const study of payload.studies || (payload.id ? [payload] : [])) {
                  if (/^3ca:[A-Za-z0-9_-]{1,60}$/.test(study.id) && !job.catalog_references.some((entry) => entry.id === study.id))
                    job.catalog_references.push({ id: study.id, title: String(study.title || "") });
                }
