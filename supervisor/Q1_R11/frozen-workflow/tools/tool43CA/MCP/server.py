"""stdio MCP facade for the workflow-local threeca-access package."""
...
os.environ.setdefault(
    "THREECA_CACHE",
    str(Path(__file__).resolve().parents[3] / ".threeca" / "cache"),
)
...
@mcp.tool()
def crawl_site(max_pages: int = 0, force: bool = False) -> dict:
...
@mcp.tool()
def download_asset(target: str, kind: str = "", extract: bool = False) -> dict:
    """Download a cataloged 3CA file into the fixed local cache with SHA-256 provenance and optional safe extraction."""
    return threeca.download_asset(target, kind, extract=extract)


@mcp.tool()
def inspect_dataset(path: str, sample_rows: int = 3) -> dict:
    """Inspect a cached archive/table, or list actual directory entries; a listing does not validate matrix contents."""
