...
try {
    if ($Probe) {
        Write-Output "workflow=$workflowRoot task=$taskRoot max_rounds=0 research_gate=required"
        return
    }
    & (Join-Path $workflowRoot 'run-Qwen3.5-4B.ps1') --workspace $taskRoot --allow-write --allow-shell --autonomous --max-rounds 0 --prompt-file (Join-Path $PSScriptRoot 'prompt.txt') --require-artifact 'report/main.tex' --require-artifact 'report/main.pdf' --require-artifact 'results/summary.json' --require-artifact 'results/analysis_manifest.json' --require-artifact 'README.md'
