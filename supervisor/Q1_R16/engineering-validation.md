...
- Explicit cluster-size/resolution/random-count/confounder/within-replicate core facts and hashes: passed
- Illustrated report contract (two figures, booktabs table, formula, labels/captions/prose refs): passed
- Current LaTeX log and Poppler all-page rendering/layout gate: passed
- Status-only fake compile/validation rejection: passed
- R16_LAUNCH_STATUS=ready

The test establishes tool operability, not the correctness of the future R16 biological conclusion.

At approximately 2026-09-13 17:23 +08:00, the latest complete test-Qwen3.5-4B.ps1 exited 0 after adding explicit report facts, semantic prose checks, the illustrated-writing contract, current LaTeX-log checks and Poppler all-page PDF rendering. The focused guard/recovery suite and skill-creator quick validation independently exited 0. All 18 external MCP entry points, synthetic core/bundle rejection, live structured sources, model, attachment, file, shell, network and isolation checks passed. The network-smoke transcript is workflow_codex/.runtime/codex-home/sessions/2026/09/13/rollout-2026-09-13T17-22-42-281e4eb2-b95.jsonl; its generic news content is not scientific evidence.

R12 through R15 are preserved as failed, incomplete rounds. R16 is a new empty workspace with a distinct persisted-task key and no resume. For research runs, the shared execution layer blocks custom workspace Python until the vetted core result exists; after that it still requires audit_analysis_code to accept the exact current file SHA-256, and editing invalidates authorization. The bundle gate now rejects R15's observed prose and layout failure modes. Engineering checks do not establish biological acceptance.
