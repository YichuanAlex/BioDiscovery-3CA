...
- R17_LAUNCH_STATUS=ready

The test establishes tool operability, not the correctness of the future R17 biological conclusion.
...
R12 through R16 are preserved as failed, incomplete rounds. R17 is a new empty workspace with a distinct persisted-task key and no resume. For research runs, the shared execution layer blocks custom workspace Python until the vetted core result exists; after that it still requires audit_analysis_code to accept the exact current file SHA-256, and editing invalidates authorization. The bundle gate rejects the observed prose and layout failure modes. Engineering checks do not establish biological acceptance.
