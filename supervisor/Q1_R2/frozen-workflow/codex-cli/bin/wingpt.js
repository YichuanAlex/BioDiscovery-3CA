...
if (!Number.isInteger(maxRounds) || maxRounds < 1 || maxRounds > 500) throw new Error("--max-rounds must be between 1 and 500.");
...
function compactContext(force = false) {
  ...
  const index = job ? JSON.stringify({ phase: job.phase, next_action: job.next_action, artifacts: job.artifacts, evidence: job.evidence.slice(-8) }) : "";
...
async function executeTool(name, args) {
  ...
  if (name === "run_powershell" && allowShell) {
    ...
    return new Promise((resolve, reject) => {
      ...
      let output = "";
      const collect = (chunk) => { output = (output + chunk.toString("utf8")).slice(-58000); };
      child.stdout.on("data", collect); child.stderr.on("data", collect);
      const progress = setInterval(() => {
        console.log(`[tool-progress] run_powershell pid=${child.pid}`);
        writeSessionEvent("tool_progress", { name, call_id: currentCallId, pid: child.pid, output_tail: output.slice(-2000) });
      }, 30000);
...
The project-local 3CA tools are enabled. For 3CA tasks call search_studies first, and plan_asset before a large download.
...
if (autonomous) {
  ...
  if (resume) {
    ...
    writeSessionEvent("task_resumed", { previous_session: job.session_file, job_path: jobPath, previous_status: job.status });
    compactContext(true);
  } else {
    ...
    job = { model, workspace, prompt, required_artifacts: requiredArtifacts, phase: "start", next_action: "inspect available tools and task sources", artifacts: [], evidence: [], rounds: 0, status: "running" };
  }
}
...
async function runTurn(userPrompt) {
  ...
  let emptyReplies = 0;
  let toolErrors = 0;
  let enableThinking = !/^\/no_think\b/.test(userPrompt);
  let noActionReplies = 0;
  let stalledRounds = 0;
  saveJob("running");
  for (let turn = 0; turn < maxRounds; turn += 1) {
    compactContext();
    const activeTools = tools.filter((tool) => webSearches < 3 || tool.function.name !== "web_search");
    const data = await request({
      ...
      temperature: 0.2,
      chat_template_kwargs: { enable_thinking: enableThinking && !answerOnly },
    });
    ...
    writeSessionEvent("assistant_response", {
      ...
      generation: { thinking_enabled: enableThinking && !answerOnly, max_tokens: answerOnly ? 512 : 4096, finish_reason: data.choices?.[0]?.finish_reason, usage: data.usage },
    });
    ...
    if (!calls.length) {
      ...
      if (autonomous && job.status !== "completed_candidate") {
        ...
        if (++noActionReplies > 6) { saveJob("needs_attention"); throw new Error("Autonomous task stalled: six answers without an action; checkpoint preserved."); }
    ...
    for (const call of calls) {
      ...
      let result;
      try {
        if (!enabledToolNames.has(name)) throw new Error(`Tool is not enabled: ${name}`);
        ...
        const key = `${name}:${JSON.stringify(args || {})}${verification ? `:${workspaceVersion}` : ""}`;
        const attempts = seenToolCalls.get(key) || 0;
        if (attempts >= (verification ? 2 : 1)) {
          result = "This identical tool call was already attempted. Use its actual earlier result; do not assume it succeeded. Continue with a different necessary action or describe the unresolved issue.";
          ...
          if (["write_file", "append_file", "replace_in_file"].includes(name) || (before !== null && before !== workspaceStamp())) workspaceVersion += 1;
          const success = !result.startsWith("ERROR:") && !/(?:^|\n)exit_code=(?!0\b)/.test(result);
          madeProgress ||= success && name !== "task_checkpoint";
          if (job && name === "run_powershell" && success) {
      ...
      if (result.startsWith("ERROR:") || /(?:^|\n)exit_code=(?!0\b)/.test(result)) { toolErrors += 1; enableThinking = false; }
      else toolErrors = 0;
    ...
    if (stalledRounds >= 8 || toolErrors >= 8) {
...
main().catch((error) => {
  if (job) { job.last_error = error.message || String(error); saveJob(job.status === "running" ? "interrupted" : job.status); }
