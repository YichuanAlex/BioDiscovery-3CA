...
TASK_ROOT="$PROJECT_ROOT/3CA/Q1_R26"
SUPERVISOR_ROOT="$PROJECT_ROOT/supervisor/Q1_R26"
PROMPT="$SUPERVISOR_ROOT/prompt.txt"
shopt -s nullglob dotglob
task_entries=("$TASK_ROOT"/*)
if (( ${#task_entries[@]} != 0 )); then echo "R26 must start empty." >&2; exit 64; fi
...
if [[ -e "$CODEX_HOME/tasks/$TASK_HASH.json" ]]; then echo "R26 checkpoint already exists." >&2; exit 65; fi
