...
3CA catalog 检索每个会话最多执行八次；同一工作区版本下完全相同的工具调用只执行一次。长 catalog 返回保持为可解析的压缩 JSON，包含总条目数、实际返回条目数和收窄过滤提示。达到检索上限后，模型仍可使用 `get_study`、`plan_asset`、`download_asset`、文件和 PowerShell 等工具继续任务。
