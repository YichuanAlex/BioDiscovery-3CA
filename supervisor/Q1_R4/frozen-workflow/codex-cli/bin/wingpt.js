...
function compactContext(force = false) {
  ...
  const recent = messages.slice(-24).map((item) => typeof item.content === "string"
...
The project-local 3CA tools are enabled. For 3CA tasks call search_studies first, then use stable study ids with get_study/plan_asset/download_asset. Do not enumerate categories after a viable catalog result. At most eight catalog searches are allowed; narrow filters instead of repeating broad queries. Use the direct tools while they work instead of invoking the same CLI command through PowerShell.
...
async function runTurn(userPrompt) {
  ...
  let suppressToolNextRound = null;
  let webSearches = 0;
  ...
  for (let turn = 0; turn < maxRounds; turn += 1) {
    compactContext();
    const suppressedTool = suppressToolNextRound;
    suppressToolNextRound = null;
    const activeTools = tools.filter((tool) =>
      (webSearches < 3 || tool.function.name !== "web_search") &&
      (catalogSearches < 8 || tool.function.name !== "search_studies") &&
      tool.function.name !== suppressedTool);
    if (suppressedTool) writeSessionEvent("loop_breaker", { turn_id: turnId, round: turn + 1, suppressed_tool: suppressedTool, duration_responses: 1, reason: "repeated_identical_call" });
    ...
    for (const call of calls) {
      ...
      try {
        ...
        if (attempts >= 1) {
          ...
          if (duplicateCount >= 2) suppressToolNextRound = name;
