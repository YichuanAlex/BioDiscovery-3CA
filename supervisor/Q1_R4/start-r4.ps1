...
if (Test-Path -LiteralPath $manifestPath) { throw 'R4 already has a launch manifest; refusing a duplicate.' }
if (@(Get-ChildItem -LiteralPath $taskRoot -Force).Count -ne 0) { throw 'R4 research workspace is not empty; refusing a contaminated fresh run.' }
...
$process = Start-Process -FilePath 'C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe' -ArgumentList @('-NoProfile', '-File', (Join-Path $PSScriptRoot 'launch-r4.ps1')) -WindowStyle Hidden -PassThru -RedirectStandardOutput (Join-Path $PSScriptRoot 'logs\worker.stdout.log') -RedirectStandardError (Join-Path $PSScriptRoot 'logs\worker.stderr.log')
...
Write-Host "Started project-local Qwen3.5-4B R4 runner PID $($process.Id)."
