...
const task = "C:\\Users\\User\\Desktop\\agentic\\3CA\\Q1_R5";
...
let originalInputs = 0, engineContinuations = 0, reasoningPresent = 0, reasoningMissing = 0, thinkingEnabled = 0, loopBreakers = 0, loopRecoveries = 0;
...
const audit = { snapshot_at: new Date().toISOString(), task, status: runner?.runner_success === false ? "runner_failed" : runner?.runner_success === true ? "runner_finished" : job?.status || "not_started", runner_alive: runnerAlive, checkpoint_status: job?.status, phase: job?.phase, next_action: job?.next_action, rounds: job?.rounds, last_error: job?.last_error, sessions, tools: rows.length, tool_counts: toolCounts, failed_tools: rows.filter(row => row[5] === "failed").length, guarded_tools: rows.filter(row => row[5] === "guarded").length, loop_breakers: loopBreakers, loop_recoveries: loopRecoveries, original_user_inputs: originalInputs, supervisor_research_interventions: Math.max(0, originalInputs - 1), engine_continuations: engineContinuations, thinking_enabled_responses: thinkingEnabled, reasoning_present_responses: reasoningPresent, reasoning_missing_responses: reasoningMissing, artifacts, runner, run_until_complete: manifest?.run_until_complete === true, max_rounds: manifest?.max_rounds, frozen_hash_mismatches: frozenHashMismatches.map(item => item.file), semantic_acceptance: runner?.runner_success === false ? "not_accepted" : "pending_independent_review", supervisor_operational_restarts: manifest?.supervisor_operational_restarts || 0, checkpoint_read_error: checkpointReadError, caveat: "Tool return, exit 0 and artifact existence do not establish scientific correctness. Missing reasoning is not reconstructed. Persisted state is not process liveness." };
...
console.log(JSON.stringify({ status: audit.status, runner_alive: runnerAlive, phase: audit.phase, rounds: audit.rounds, tool_results: audit.tools, tool_counts: toolCounts, failed_tools: audit.failed_tools, guarded_tools: audit.guarded_tools, loop_breakers: loopBreakers, loop_recoveries: loopRecoveries, engine_continuations: audit.engine_continuations, supervisor_research_interventions: audit.supervisor_research_interventions, thinking_enabled_responses: thinkingEnabled, reasoning_present_responses: reasoningPresent, artifacts: artifacts.length, run_until_complete: audit.run_until_complete, max_rounds: audit.max_rounds, frozen_hash_mismatches: audit.frozen_hash_mismatches, runner }, null, 2));
