  ...
  for (let turn = 0; runUntilComplete || turn < maxRounds; turn += 1) {
    ...
    if (stalledRounds >= 8 || toolErrors >= 8) {
      if (autonomous && runUntilComplete) {
        ...
        writeSessionEvent("loop_recovery", { turn_id: turnId, round: turn + 1, reason: stalledRounds >= 8 ? "eight_rounds_without_progress" : "eight_tool_errors" });
        stalledRounds = 0; toolErrors = 0;
        compactContext(true); saveJob("running");
        continue;
      }
