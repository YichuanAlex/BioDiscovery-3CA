...
try {
    ...
    while ($true) {
        ...
        if ((-not $runUntilComplete -and $attempt -ge $attemptLimit) -or -not $checkpoint -or $checkpoint.status -ne 'interrupted' -or $checkpoint.last_error -notmatch 'API (429|5\d\d)|fetch failed|timeout|abort|ECONN|socket|network|ENOSPC|EBUSY|EPERM') {
