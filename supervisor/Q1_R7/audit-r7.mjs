...
const task = "C:\\Users\\User\\Desktop\\agentic\\3CA\\Q1_R7";
...
let originalInputs = 0, engineContinuations = 0, reasoningPresent = 0, reasoningMissing = 0, thinkingEnabled = 0, loopBreakers = 0, loopRecoveries = 0, structuredActions = 0, structuredRequests = 0;
...
const audit = { snapshot_at: new Date().toISOString(), task, status: runner?.status === "stopped_by_user" ? "stopped_by_user" : runner?.runner_success === false ? "runner_failed" : runner?.runner_success === true ? "runner_finished" : (job?.status === "running" && !runnerAlive ? "runner_missing" : job?.status || "not_started"), runner_alive: runnerAlive, checkpoint_status: job?.status, phase: job?.phase, next_action: job?.next_action, rounds: job?.rounds, catalog_searches: job?.catalog_searches || 0, web_searches: job?.web_searches || 0, catalog_references: job?.catalog_references || [], discovery_cooldown: job?.discovery_cooldown || 0, last_error: job?.last_error, sessions, tools: rows.length, tool_counts: toolCounts, failed_tools: rows.filter(row => row[5] === "failed").length, guarded_tools: rows.filter(row => row[5] === "guarded").length, loop_breakers: loopBreakers, loop_recoveries: loopRecoveries, structured_actions: structuredActions, structured_requests: structuredRequests, original_user_inputs: originalInputs, supervisor_research_interventions: Math.max(0, originalInputs - 1), engine_continuations: engineContinuations, thinking_enabled_responses: thinkingEnabled, reasoning_present_responses: reasoningPresent, reasoning_missing_responses: reasoningMissing, artifacts, required_artifacts: requiredArtifacts, free_bytes: freeBytes, runner, run_until_complete: manifest?.run_until_complete === true, max_rounds: manifest?.max_rounds, frozen_hash_mismatches: frozenHashMismatches.map(item => item.file), semantic_acceptance: runner?.runner_success === false ? "not_accepted" : "pending_independent_review", supervisor_operational_restarts: manifest?.supervisor_operational_restarts || 0, checkpoint_read_error: checkpointReadError, caveat: "Tool return, exit 0 and artifact existence do not establish scientific correctness. Missing reasoning is not reconstructed. Persisted state is not process liveness." };
audit.structural_completion_candidate = runner?.runner_success === true && job?.status === "completed_candidate" && requiredArtifacts.every(item => item.present) && audit.frozen_hash_mismatches.length === 0;
