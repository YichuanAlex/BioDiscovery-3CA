...
function compactContext(force = false, recovery = false) {
  ...
  const index = job ? boundedModelText(JSON.stringify({ budgets: { catalog_searches: job.catalog_searches || 0, catalog_limit: 8, web_searches: job.web_searches || 0, web_limit: 3 }, catalog_references: job.catalog_references || [], discovery_cooldown: job.discovery_cooldown || 0, phase: job.phase, next_action: job.next_action, artifacts: job.artifacts, evidence: job.evidence.slice(-8), observations: (job.observations || []).slice(-16) }), 12000) : "";
...
async function runTurn(userPrompt) {
  ...
  for (let turn = 0; runUntilComplete || turn < maxRounds; turn += 1) {
    ...
    const runtimeContext = autonomous ? JSON.stringify({
      ...
      workspace_version: workspaceVersion, catalog_references: job.catalog_references || [], discovery_cooldown: discoveryCooldown,
