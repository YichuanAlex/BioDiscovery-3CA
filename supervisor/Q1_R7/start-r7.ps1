...
$taskRoot = 'C:\Users\User\Desktop\agentic\3CA\Q1_R7'
$manifestPath = Join-Path $PSScriptRoot 'run-manifest.json'
if (Test-Path -LiteralPath $manifestPath) { throw 'R7 already has a launch manifest; refusing a duplicate.' }
if (@(Get-ChildItem -LiteralPath $taskRoot -Force).Count -ne 0) { throw 'R7 research workspace is not empty; refusing a contaminated fresh run.' }
...
$process = Start-Process -FilePath 'C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe' -ArgumentList @('-NoProfile', '-File', (Join-Path $PSScriptRoot 'launch-r7.ps1')) -WindowStyle Hidden -PassThru -RedirectStandardOutput (Join-Path $PSScriptRoot 'logs\worker.stdout.log') -RedirectStandardError (Join-Path $PSScriptRoot 'logs\worker.stderr.log')
$manifest = @{ started_at = $started; workflow = $workflowRoot; task = $taskRoot; supervisor = $PSScriptRoot; runner_pid = $process.Id; model = 'Qwen3.5-4B'; max_rounds = 0; run_until_complete = $true; prompt_sha256 = (Get-FileHash -LiteralPath (Join-Path $PSScriptRoot 'prompt.txt') -Algorithm SHA256).Hash.ToLowerInvariant(); frozen_files = @($frozen); engineering_tests = 'passed'; research_workspace_initially_empty = $true; supervisor_operational_restarts = 0; supervisor_research_interventions = 0; semantic_acceptance = 'pending' }
$manifest | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath $manifestPath -Encoding UTF8
Write-Host "Started project-local Qwen3.5-4B R7 until-complete runner PID $($process.Id)."
