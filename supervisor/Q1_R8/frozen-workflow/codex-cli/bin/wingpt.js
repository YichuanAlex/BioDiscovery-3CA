...
function compactContext(force = false, recovery = false) {
  ...
  const index = job ? boundedModelText(JSON.stringify({ budgets: { catalog_searches: job.catalog_searches || 0, catalog_limit: 8, web_searches: job.web_searches || 0, web_limit: 3 }, catalog_references: job.catalog_references || [], asset_references: job.asset_references || [], discovery_cooldown: job.discovery_cooldown || 0, phase: job.phase, next_action: job.next_action, artifacts: job.artifacts, evidence: job.evidence.slice(-8), observations: (job.observations || []).slice(-16) }), 12000) : "";
...
if (autonomous) {
  messages[0].content += '\nReturn exactly one JSON action object {"name":"an advertised tool name","arguments":{...}} matching the provided structured-output schema. Do not use XML or prose outside the JSON. Include every required argument. Write long files in bounded chunks with append_file.';
...
async function runTurn(userPrompt) {
  ...
  // ponytail: retain 64 bounded actual results; older duplicate marks stay guarded within the process, assets have a separate stable index.
  ...
  for (let turn = 0; runUntilComplete || turn < maxRounds; turn += 1) {
    ...
    const discoveryCooldown = job?.discovery_cooldown || 0;
    const activeTools = tools.filter((tool) =>
      ...
      (!discoveryCooldown || !["search_studies", "refresh_catalog", "get_page", "search_cached_pages", "crawl_site", "web_search", "read_webpage"].includes(tool.function.name)));
    const runtimeContext = autonomous ? JSON.stringify({
      source: "workflow_engine_runtime_state_not_a_new_task", budgets: { catalog_searches: catalogSearches, catalog_limit: 8, web_searches: webSearches, web_limit: 3 },
      workspace_version: workspaceVersion, catalog_references: job.catalog_references || [], asset_references: job.asset_references || [], discovery_cooldown: discoveryCooldown,
      advertised_tools: activeTools.map((tool) => tool.function.name), checkpoint_claims_are_not_proof: true,
    }) : "";
    if (autonomous) writeSessionEvent("model_request", { turn_id: turnId, round: turn + 1, action_mode: "structured_json", runtime_context: runtimeContext });
    ...
    const data = await request({
      ...
      ...(autonomous ? { structured_outputs: { json: { anyOf: activeTools.map(({ function: fn }) => ({
        type: "object", properties: { name: { type: "string", const: fn.name }, arguments: fn.parameters },
    ...
    if (autonomous && !reply.tool_calls?.length && typeof reply.content === "string") {
      ...
      } catch (error) { writeSessionEvent("action_parse_error", { turn_id: turnId, round: turn + 1, error: error.message }); }
    }
    if (job && discoveryCooldown) job.discovery_cooldown = discoveryCooldown - 1;
    ...
    for (const call of calls) {
      ...
      try {
        ...
        // ponytail: recognize simple CLI clauses, not a full PowerShell parser; unrelated help output must not bypass the budget.
        ...
        if (attempts >= 1) {
          ...
          if (workspaceChanged) { workspaceVersion += 1; if (job) job.discovery_cooldown = 0; }
            ...
            if (threeCaTools.handles(name)) {
              try {
                ...
                for (const study of payload.studies || (payload.id ? [payload] : [])) {
                  if (/^3ca:[A-Za-z0-9_-]{1,60}$/.test(study.id) && !job.catalog_references.some((entry) => entry.id === study.id) && job.catalog_references.length < 24)
                    job.catalog_references.push({ id: study.id, title: String(study.title || "").slice(0, 100) });
                }
                if (payload.path && payload.sha256 && /^[a-f0-9]{64}$/i.test(payload.sha256)) {
                  ...
                  const candidate = [...job.asset_references];
                  if (index >= 0) candidate[index] = { ...candidate[index], ...asset }; else candidate.push(asset);
                  // ponytail: stable source memory is capped at 16 assets / 6000 characters; full provenance remains in the rollout and manifests.
                  if (candidate.length <= 16 && JSON.stringify(candidate).length <= 6000) job.asset_references = candidate;
                  else writeSessionEvent("asset_reference_limit", { path: payload.path, source_call_id: call.id });
                }
      ...
      if (executed) {
        ...
        previousResults.splice(0, Math.max(0, previousResults.length - 64));
      }
    ...
    if (stalledRounds >= 8 || toolErrors >= 8 || explorationStalled) {
      if (autonomous && runUntilComplete) {
        ...
        // ponytail: four responses is a reversible discovery cooldown, not a permanent research restriction; tune from observed loops.
        if (job.catalog_references?.length) job.discovery_cooldown = 4;
