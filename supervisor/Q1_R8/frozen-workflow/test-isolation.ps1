...
$activeFiles = @(
    (Join-Path $root 'run-Qwen3.5-4B.ps1'),
    (Join-Path $root 'start-Qwen3.5-4B-server.ps1'),
    (Join-Path $root 'codex-cli\bin\codex.js'),
    (Join-Path $root 'codex-cli\bin\Qwen3.5-4B.js'),
    (Join-Path $root 'codex-cli\bin\Qwen3.5-4B-computer-use.js'),
    (Join-Path $root 'codex-cli\bin\Qwen3.5-4B-threeca.js'),
    (Join-Path $root 'codex-local.cmd'),
    (Join-Path $root 'app-server'),
...
if ($localCodexEntry -notmatch 'Qwen3.5-4B\.js' -or $localCodexEntry -notmatch 'CODEX_HOME' -or $localCodexEntry -notmatch 'THREECA_CACHE' -or $localCodexEntry -match 'codex\.exe') {
    throw 'Project codex entry does not exclusively route to the local Qwen3.5-4B agent.'
}

$localArtifacts = @(
    (Join-Path $root 'tools\computer-use\node_modules\@oai\sky\package.json'),
...
$links = Get-ChildItem -LiteralPath (Join-Path $root 'tools\computer-use'), (Join-Path $root 'tools\tool43CA') -Recurse -Force -Attributes ReparsePoint -ErrorAction SilentlyContinue
