...
$taskRoot = 'C:\Users\User\Desktop\agentic\3CA\Q1_R8'
$manifestPath = Join-Path $PSScriptRoot 'run-manifest.json'
if (Test-Path -LiteralPath $manifestPath) { throw 'R8 already has a launch manifest; refusing a duplicate.' }
if (-not (Test-Path -LiteralPath $taskRoot)) { New-Item -ItemType Directory -Path $taskRoot | Out-Null }
if (@(Get-ChildItem -LiteralPath $taskRoot -Force).Count -ne 0) { throw 'R8 research workspace is not empty; refusing a contaminated fresh run.' }
$validation = Get-Content -LiteralPath (Join-Path $PSScriptRoot 'engineering-validation.md') -Raw -Encoding UTF8
if (-not $validation.Contains('FULL_TEST_EXIT_CODE=0') -and -not ($validation.Contains('CORE_TEST_EXIT_CODE=0') -and $validation.Contains('R8_DESKTOP_TOOLS=not_exposed'))) { throw 'Research core tests have not completed successfully.' }
