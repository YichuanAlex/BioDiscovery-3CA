...
它通过本地 vLLM 使用 `C:\Users\User\Desktop\agentic\model\Qwen3.5-4B` 中的 `Qwen3.5-4B`（历史 Qwen3.5-4B 入口文件名保留兼容），不调用电脑中安装的 `codex` 命令，也不会自动扫描或写回 `C:\Users\User\.codex`、`C:\Users\User\.agents` 或 `%LOCALAPPDATA%\OpenAI\Codex`。只有当用户在提示中明确给出外部附件路径时，程序才会读取该源文件并复制一份到本项目；运行时状态、3CA 缓存和 Computer Use 运行时均保存在本项目内。
...
完整测试包含：路径隔离、当前 Qwen 模型、文件工具、联网工具、3CA CLI/MCP，以及 Computer Use 的应用枚举、启动、窗口选择、观察、按键动作和动作后观察。Windows 桌面已锁定时，输入动作会明确标记为 `SKIP`；解锁后重新运行即可强制验证真实动作，测试不会尝试越过登录界面。

`test-Qwen3.5-4B.ps1 -SkipDesktopAction` 仅用于独立验证非桌面核心路径，明确输出桌面集成被排除，不代表完整测试通过。桌面动作失败仍应保留原始错误与完整测试退出码；科研任务不暴露桌面工具时可单独记录核心验证，不能据此声称桌面能力正常。联网冒烟最多六轮，要求一次实际检索及带来源的可见答复，超限/停滞不算通过。测试脚本使用 UTF-8 BOM 兼容 Windows PowerShell 5 的中文文本；长任务提示通过 `--prompt-file` 显式 UTF-8 读取，不依赖 shell 的中文字面量编码。
...
Windows 11 Insider build 26200 上，项目内 Node 24 长任务统一从启动器加 `--no-maglev`，规避实际观察到的 `0xC0000409` V8 Maglev FailFast；不改变模型权重、工具权限或研究策略。若以后更换到已验证不受影响的 Node/Windows 组合，再以长程回归证据移除此兼容参数。

自主模式复用当前可用工具的参数定义，通过本地 vLLM `structured_outputs.json` 约束每次回复为一个 JSON 行动；交互模式保留原有原生工具调用。执行入口检查必填参数、类型和当前可用工具，缺少文件内容时不会先创建目录。日志分别保存未修改的服务响应和从实际 JSON 解码的行动，保留服务实际返回的 reasoning，不冒充服务原生工具调用。

3CA catalog 检索每个任务最多执行八次；CLI `search --help` / `-h` 是帮助检查，不消耗检索额度；同一工作区版本下完全相同的工具调用只执行一次，JSON 参数键顺序不同也视为同一调用。相同调用第二次被护栏拒绝后，只把该工具从下一次模型响应中临时隐藏一轮；catalog/web 预算拒绝和真实工具错误同样冷却对应工具一轮，随后自动恢复，避免永久阻断合法验证。长 catalog 返回保持为可解析的压缩 JSON，包含总条目数、实际返回条目数和收窄过滤提示。达到检索上限后，`search_studies` 不再出现在后续工具列表中，但模型仍可使用 `get_study`、`plan_asset`、`download_asset`、文件和 PowerShell 等工具继续任务。仅包含数据路径的 PowerShell 输入会被明确拒绝；空输出、无工作区变化的 exit 0 不再算作进展或完成验证。

在 until-complete 模式中，即使搜索或 shell 每次都有新输出，连续 12 个工具轮次没有工作区修改仍会触发上下文恢复，不因此退出任务；恢复不能替代科学规划。catalog/web 预算跨 resume 保存，不会因进程恢复重置。从实际工具返回中机械保留最多 24 个研究 ID 和短标题，不替模型选择研究对象；预算、这些来源引用和当前工具列表每次请求都可见。有已知来源但陷入循环时，发现类工具暂时冷却四次响应；研究详情、资产、文件和执行工具仍可用，冷却期结束或工作区修改后恢复。四次响应是可调的通用启发式，不保证科研进展。启动器只读取当前工作区的检查点，并持有操作系统独占文件锁，进程退出后自动释放，避免并行运行同一任务。上下文证据索引最多 12000 字符，验证证据只保留最近 32 项。识别桌面任务前先剔除本地路径，路径中的 `Desktop` 不会误启用 Computer Use；恢复时依据原始任务判断。
...
重复调用在有限缓存中回用原实际结果（最多 64 项，每项最多 6000 字符），明确标注原会话、原调用 ID、时间及未重新执行，不把回用当成新验证或新进展。恢复及 resume 保留这些结果；缺少结果的历史调用仍不盲目重放副作用。资产来源另行保留最多 16 项、合计最多 6000 字符的实际下载路径、来源 URL、字节数、SHA-256 和解压路径，每次请求均可见，不依赖轮转的近期消息；这只是机械记忆，不代选数据集，也不校验生物学结论。
...
Qwen 服务直接加载指定本地目录，文本任务使用 language-model-only，原生工具解析器 qwen3_coder、reasoning 解析器 qwen3；自主任务继续使用结构化 JSON 行动并保留真实 reasoning。当前硬件下上下文长度为 65536，未启用视觉模块，也未修改权重或全局已安装的 agent。
