...
$taskRoot = 'C:\Users\User\Desktop\agentic\3CA\Q1_R9'
$manifestPath = Join-Path $PSScriptRoot 'run-manifest.json'
if (Test-Path -LiteralPath $manifestPath) { throw 'R9 already has a launch manifest; refusing a duplicate.' }
if (-not (Test-Path -LiteralPath $taskRoot)) { New-Item -ItemType Directory -Path $taskRoot | Out-Null }
if (@(Get-ChildItem -LiteralPath $taskRoot -Force).Count -ne 0) { throw 'R9 research workspace is not empty; refusing a contaminated fresh run.' }
$validation = Get-Content -LiteralPath (Join-Path $PSScriptRoot 'engineering-validation.md') -Raw -Encoding UTF8
if (-not $validation.Contains('FULL_TEST_EXIT_CODE=0') -and -not ($validation.Contains('CORE_TEST_EXIT_CODE=0') -and $validation.Contains('R9_DESKTOP_TOOLS=not_exposed'))) { throw 'Research core tests have not completed successfully.' }
...
$process = Start-Process -FilePath 'C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe' -ArgumentList @('-NoProfile', '-File', (Join-Path $PSScriptRoot 'launch-r9.ps1')) -WindowStyle Hidden -PassThru -RedirectStandardOutput (Join-Path $PSScriptRoot 'logs\worker.stdout.log') -RedirectStandardError (Join-Path $PSScriptRoot 'logs\worker.stderr.log')
$manifest = @{ started_at = $started; workflow = $workflowRoot; task = $taskRoot; supervisor = $PSScriptRoot; runner_pid = $process.Id; model = 'Qwen3.5-4B'; model_directory = $modelDirectory; model_identity = $modelIdentity[0]; model_metadata = @($modelMetadata); max_rounds = 0; run_until_complete = $true; prompt_sha256 = (Get-FileHash -LiteralPath (Join-Path $PSScriptRoot 'prompt.txt') -Algorithm SHA256).Hash.ToLowerInvariant(); frozen_files = @($frozen); engineering_tests = $(if ($validation.Contains('FULL_TEST_EXIT_CODE=0')) { 'full_passed' } else { 'core_passed_desktop_action_failed' }); desktop_action_test = $(if ($validation.Contains('FULL_TEST_EXIT_CODE=0')) { 'passed' } else { 'failed_activation_not_used_by_R9' }); research_workspace_initially_empty = $true; supervisor_operational_restarts = 0; supervisor_research_interventions = 0; semantic_acceptance = 'pending' }
$manifest | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath $manifestPath -Encoding UTF8
Write-Host "Started project-local Qwen R9 until-complete runner PID $($process.Id)."
