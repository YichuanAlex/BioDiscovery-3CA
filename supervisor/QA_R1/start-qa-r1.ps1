...
$taskRoot = 'C:\Users\User\Desktop\agentic\3CA\QA_R1'
...
if (Test-Path -LiteralPath $manifestPath) { throw 'QA_R1 already has a launch manifest; refusing a duplicate.' }
if (-not (Test-Path -LiteralPath $taskRoot -PathType Container)) { throw 'QA_R1 task workspace is missing.' }
if (@(Get-ChildItem -LiteralPath $taskRoot -Force).Count -ne 0) { throw 'QA_R1 research workspace is not empty before supplied-input staging.' }
