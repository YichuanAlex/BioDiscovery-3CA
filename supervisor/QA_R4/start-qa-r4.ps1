...
$taskRoot = 'C:\Users\User\Desktop\agentic\3CA\QA_R4'
...
if (Test-Path -LiteralPath $manifestPath) { throw 'QA_R4 already has a launch manifest; refusing a duplicate.' }
if (-not (Test-Path -LiteralPath $taskRoot -PathType Container)) { throw 'QA_R4 task workspace is missing.' }
if (@(Get-ChildItem -LiteralPath $taskRoot -Force).Count -ne 0) { throw 'QA_R4 research workspace is not empty before supplied-input staging.' }
