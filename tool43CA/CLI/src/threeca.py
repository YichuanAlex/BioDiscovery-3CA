...
DEFAULT_MAX_EXTRACT_BYTES = 50 * 1024**3
...
def cache_root(value: str | Path | None = None) -> Path:
    if value:
        ...
        root = Path.home() / ".agents" / "cache" / "threeca"
...
def crawl_site(cache: str | Path | None = None, max_pages: int = 1500, workers: int = 4, force: bool = False) -> dict[str, object]:
    if max_pages < 1:
        raise ThreeCAError("max_pages must be positive.")
    root = cache_root(cache)
    ...
    while queue and len(seen) < max_pages:
        batch = [queue.popleft() for _ in range(min(max(1, workers), max_pages - len(seen), len(queue)))]
        with ThreadPoolExecutor(max_workers=max(1, min(workers, 8))) as pool:
                ...
                try:
                    ...
                    for link in page["links"]:  # type: ignore[index]
                        try:
                            ...
                            continue
                        if child not in seen and child not in queued and len(queued) < max_pages * 2:
                            queued.add(child)
                            queue.append(child)
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--cache", help="Cache directory (default: ~/.agents/cache/threeca)")
    ...
    crawl.add_argument("--max-pages", type=int, default=1500)
