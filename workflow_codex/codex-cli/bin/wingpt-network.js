...
export function createNetworkTools(functionTool) {
  const definitions = [
    ...
    functionTool(
      "read_webpage",
      "Read text from a public HTTP or HTTPS page. Private-network and localhost URLs are blocked.",
      {
        url: { type: "string" },
...
async function fetchPublicText(input, maxBytes = 64 * 1024 * 1024) {
  let current = await publicUrl(input);
  for (let redirects = 0; redirects <= 10; redirects += 1) {
    const response = await fetch(current, {
      redirect: "manual",
      headers: { "User-Agent": "WiNGPT-local-agent/1.0" },
    });
...
async function webSearch(args) {
  ...
  const limit = Math.max(1, Number(args.max_results) || 10);
...
async function readWebpage(args) {
  const maxChars = Math.max(1000, Number(args.max_chars) || 120000);
