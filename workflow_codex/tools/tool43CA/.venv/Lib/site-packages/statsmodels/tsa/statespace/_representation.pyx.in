...
cdef class {{prefix}}Statespace(object):
    ...
    *See Durbin and Koopman (2012), Chapter 4 for all notation*
    ...
    def __init__(self,
        ...
        dim1[0] = self.k_endog;
        self.transform_obs_intercept = np.PyArray_ZEROS(1, dim1, {{typenum}}, FORTRAN)
    ...
    def initialize_stationary(self, complex_step=False):
        ...
        if asum > tol:
            ...
            blas.{{prefix}}copy(&k_states2, &self.transition[0,0,0], &inc,
                                            &self.tmp[0,0], &inc)
            scalar = -1.0
            blas.{{prefix}}scal(&k_states2, &scalar, &self.tmp[0, 0], &inc)
        ...
        # covariance matrix
        tools._{{prefix}}solve_discrete_lyapunov(&self.tmp[0,0], &self.initial_state_cov[0,0], self.k_states, complex_step)
    ...
    cdef void transform_diagonalize(self, unsigned int t, unsigned int previous_t, unsigned int reset=False) except *:
        ...
        cdef:
            ...
            {{cython_type}} * _transform_cholesky = &self.transform_cholesky[0, 0]
        ...
        if t == 0 or self.obs_cov.shape[2] > 1 or reset_missing or reset:
            ...
            for i in range(self._k_endog):
                ...
                if not self.transform_cholesky[k, l] == 0:
                    self.transform_determinant = self.transform_determinant + {{combined_prefix}}log(self.transform_cholesky[k, l])
